...
groups shipped entries by a field (default platform), and reports each
group's hit rate. Groups below --min-n are marked insufficient_data rather
than given a misleadingly confident number. Use the output to lean which
idea-to-platform mapping and hook family to reach for first — never to
silently skip a platform the user asked for, and always disclose the note
in the packet's output.
...
def main():
    ...
    parser.add_argument("--group-by", default="platform", help="Field to group by (default: platform)")
